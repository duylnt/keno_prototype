(function () {
  ...
  function paintCountdown() {
    document.querySelectorAll("[data-pos-cd]").forEach((el) => {
      el.textContent = fmt(countdown);
    });
    document.querySelectorAll("[data-pos-next]").forEach((el) => {
  ...
  function showScreen(id) {
    document.querySelectorAll(".pos-screen").forEach((el) => {
  ...
  function paintMeta() {
    paintCountdown();
    document.querySelectorAll("[data-pos-period]").forEach(function (el) {
  ...
  function playCurrent() {
    ...
    const after = function () {
      if (paused) return;
      timer = setTimeout(next, spec.ms || 400);
    };
    if (spec.animate === "live") runLive().then(after);
    else if (spec.animate === "grid80") runGrid80().then(after);
    else timer = setTimeout(next, spec.ms);
  }
  ...
  function prev() {
    idx = (idx - 1 + LOOP.length) % LOOP.length;
    playCurrent();
  }
  ...
  function applyPayload(nextData) {
    ...
    if (data.current_period && prevPeriod && data.current_period !== prevPeriod) {
      idx = LOOP.findIndex(function (s) {
        return s.id === "live-draw";
      });
      if (idx < 0) idx = 0;
      playCurrent();
    }
